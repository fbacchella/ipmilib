...
public class SyncApiTest {
    ...
    public void testGetChannelAuthenticationCapabilities() throws Exception {
        ...
        List<CipherSuite> ciphers = connector.getAvailableCipherSuites(connection);

        CipherSuite cs = null;
        if (ciphers.size() > STANDARD_CIPHER_SUITE) {
            cs = ciphers.get(STANDARD_CIPHER_SUITE);
        } else {
            cs = ciphers.get(STANDARD_CIPHER_SUITE - 1);
        }
        connector.getChannelAuthenticationCapabilities(connection, cs, PrivilegeLevel.User);
    ...
    /**
     * Tests
     * {@link IpmiConnector#sendMessage(ConnectionHandle, com.veraxsystems.vxipmi.coding.PayloadCoder)}
     *
     * @throws Exception
     */
    @Test
